...
SELECT TOP 5 Player_Name, Runs_Scored FROM dbo.IPL_Dataset WHERE Year = 2023 ORDER BY Runs_Scored DESC
...
FROM dbo.IPL_Dataset
WHERE Year = 2023

--3. Find the player with the highest batting strike rate in a particular IPL season?
SELECT TOP 1 Player_Name, Batting_Strike_Rate FROM dbo.IPL_Dataset WHERE Year = 2023 ORDER BY Batting_Strike_Rate DESC

--4. Identify the players who scored centuries (100 runs or more) in a specific IPL season?
SELECT Player_Name, Centuries FROM dbo.IPL_Dataset WHERE Year = 2023 AND Centuries > 0

--5. Find the players who have taken the most catches as fielders in a particular IPL season?
SELECT Player_Name, Catches_Taken FROM dbo.IPL_Dataset WHERE Year = 2023 ORDER BY Catches_Taken DESC
...
SELECT TOP 1 Player_Name, Bowling_Average FROM dbo.IPL_Dataset WHERE Year = 2023 AND Wickets_Taken > 0
...
SELECT Player_Name, Five_Wicket_Hauls FROM dbo.IPL_Dataset WHERE Year = 2023 AND Five_Wicket_Hauls > 0

--9. Find the players who have scored both a century and a half-century in a specific IPL season?
SELECT Player_Name, Centuries, Half_Centuries FROM dbo.IPL_Dataset
...
SELECT TOP 5 Player_Name, MAX(Runs_Scored) AS Highest_Runs_Scored FROM dbo.IPL_Dataset
...
ELSE CAST(SUM(Runs_Scored) AS FLOAT) / (SUM(Matches_Batted) - SUM(Not_Outs)) END AS Batting_Average FROM dbo.IPL_Dataset

--12. Find the player with the highest career batting strike rate across all IPL seasons?
SELECT TOP 1 Player_Name, AVG(Batting_Strike_Rate) AS Career_Batting_Strike_Rate FROM dbo.IPL_Dataset GROUP BY Player_Name
...
SELECT Player_Name, SUM(Centuries) AS Total_Centuries FROM dbo.IPL_Dataset WHERE Centuries > 0 GROUP BY Player_Name

--14. Find the players who have taken the most catches as fielders across all IPL seasons?
SELECT Player_Name, SUM(Catches_Taken) AS Total_Catches_Taken FROM dbo.IPL_Dataset GROUP BY Player_Name ORDER BY Total_Catches_Taken DESC
...
FROM dbo.IPL_Dataset WHERE Balls_Bowled > 0 GROUP BY Player_Name
...
FROM dbo.IPL_Dataset WHERE Wickets_Taken > 0 GROUP BY Player_Name ORDER BY Career_Bowling_Average ASC

--17. Find the players who have achieved five-wicket hauls (5 wickets or more in an inning) in any IPL season?
SELECT Player_Name, SUM(Five_Wicket_Hauls) AS Total_Five_Wicket_Hauls FROM dbo.IPL_Dataset WHERE Five_Wicket_Hauls > 0
GROUP BY Player_Name